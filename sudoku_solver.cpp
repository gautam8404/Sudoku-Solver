...
#include <chrono>
#include <thread>
#define sl(S) std::this_thread::sleep_for(std::chrono::nanoseconds(S*100000));
#else
#include <unistd.h>
...
const char *title = "Sudoku Solver , Hold S to decrease and F to increase speed";
...
void solve()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (IsKeyDown(KEY_F) && speed > 30)
        {
            speed = speed - 30;
        }
        if (IsKeyDown(KEY_S) && speed < 2000)
        {
            speed = speed + 100;
        }
